	...
	<div class="flex h-screen bg-gray-50 dark:bg-gray-900"
		...
		<div class="flex flex-col flex-1">
			...
			<main class="h-full pb-16 overflow-y-auto">
				...
				<form method="post" action="<c:url value="/modifierDepense"/>">
					<div class="container px-6 mx-auto grid">
						<h2
							class="my-6 text-2xl font-semibold text-gray-700 dark:text-gray-200">
							Modifier une dépense</h2>

						<!-- General elements -->
						<h4
							class="mb-4 text-lg font-semibold text-gray-600 dark:text-gray-300">
							Formulaire dépense</h4>
						<div
							class="px-4 py-3 mb-8 bg-white rounded-lg shadow-md dark:bg-gray-800">
							<input type="text" hidden="hidden" name="idDepense"
							...
							<label class="block mt-1 text-sm"> <span
								class="text-gray-700 font-medium dark:text-gray-400">
									Montant dépensé </span> <input
								class="block w-full mt-1 text-sm dark:border-gray-600 dark:bg-gray-700 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:text-gray-300 dark:focus:shadow-outline-gray form-input"
								placeholder="Montant dépensé" name="montantDepense"
								value="<c:out value="${depense.montant}"/>" />
								<span class="text-xs text-red-600 dark:text-red-400"> <c:out
										value="${depenseForm.erreurs['montantDepense']}" />
							</span>
							</label> <label class="block mt-4 text-sm"> <span
								class="text-gray-700 font-medium dark:text-gray-400">
									Date de dépense </span> <input
								class="block w-full mt-1 text-sm dark:border-gray-600 dark:bg-gray-700 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:text-gray-300 dark:focus:shadow-outline-gray form-input"
								type="date" name="dateDepense"
								value="<c:out value="${depense.dateDepense}"/>" />
								<span class="text-xs text-red-600 dark:text-red-400"> <c:out
										value="${depenseForm.erreurs['dateDepense']}" />
							</span>
							</label> <label class="block mt-4 text-sm"> <span
								class="text-gray-700 font-medium dark:text-gray-400">
									Type de dépense </span> <input
								class="block w-full mt-1 text-sm dark:border-gray-600 dark:bg-gray-700 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:text-gray-300 dark:focus:shadow-outline-gray form-input"
								placeholder="Type de dépense" name="typeDepense"
								value="<c:out value="${depense.typeDepense}"/>" />
								<span class="text-xs text-red-600 dark:text-red-400"> <c:out
										value="${depenseForm.erreurs['typeDepense']}" />
							</span>
							</label>
						</div>
						<div class="mb-8">
