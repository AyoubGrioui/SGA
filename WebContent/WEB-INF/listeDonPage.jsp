					...
					<div class="w-full overflow-hidden rounded-lg shadow-xs">
						<div class="w-full overflow-x-auto">
									...
									<c:forEach items="${donList}" var="don" varStatus="boucle">
										<tr class="text-gray-700 dark:text-gray-400">
											...
											<td class="px-4 py-3 text-sm"><c:out
													value="${don.dateDon}" /></td>
											<td class="px-4 py-3 text-sm"><c:out
													value="${don.montant}" /></td>
											<td class="px-4 py-3 text-sm"><c:choose>
													<c:when
														test="${don.getClass().getName() == 'com.sga.entities.DonCheque'}">
														<c:out value="chèque" />
													</c:when>
													<c:when
														test="${don.getClass().getName() == 'com.sga.entities.DonEspece'}">
														<c:out value="espece" />
													</c:when>
													<c:when
														test="${don.getClass().getName() == 'com.sga.entities.DonVersement'}">
														<c:out value="versement" />
													</c:when>
												</c:choose></td>
											<td class="px-4 py-3">
												<div class="flex items-center space-x-4 text-sm">
													...
													<a
														href="<c:url value="/modifierDon"><c:param name="donID" value="${don.idDon}" /></c:url>">
                          ...
                          <path
														...
														href="<c:url value="/supprimerDon"><c:param name="donID" value="${don.idDon}" /></c:url>">
